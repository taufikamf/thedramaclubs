    ...
    <table class="table w-full">
      ...
      <tbody>
        ...
        <tr v-if="data" v-for="(post, index) in data" :key="post.id">
          <th>{{ index + 1 }}</th>
          <td>
            <nuxt-link class="link link-info" :to="`/posts/${post.slug}`">{{
...
const { data, pending, refresh } = await useFetch(
  () =>
    `${runtimeConfig.public.API_URL}wp-json/wp/v2/posts?page=${page.value}&_embed=1`
);
...
export default {
  ...
  methods: {
    ...
    copyToClipboard(text) {
      this.toCopy = `${window.location.href}api/content?slug=${text}`;
      navigator.clipboard
        .writeText(this.toCopy)
        .then(() => {
          window.alert("Success copy the url");
        })
        .catch((error) => {
